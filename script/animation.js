...
const AnimateSkills = () => {
    ...
    let skills = ["React Native Developer", "Nodejs Developer", "Frontend Developer"];
    ...
    const id = setInterval(() => {
        ...
        if (indexSkills > skills.length) {
            indexSkills = 0;

        } else {

            if (skills[indexSkills]) {
                pushText(skills[indexSkills]);
                const txtSkill = document.querySelector('.txtSkills');
                txtSkill.innerHTML = "";
            } else {
                pushText(skills[1]);
        ...
        entries.forEach(entry => {
            ...
            switch (id) {
                case "c1":
                    ...
                    document.querySelector(".c1").style.strokeDashoffset = 376.99 - (376.99 / 100) * 70;
                    break;
                case "c2":
                    changePercentage(skillTextEl, 75);
                    document.querySelector(".c2").style.strokeDashoffset = 376.99 - (376.99 / 100) * 75;
                    break;
                case "c3":
                    changePercentage(skillTextEl, 60);
                    document.querySelector(".c3").style.strokeDashoffset = 376.99 - (376.99 / 100) * 60;
                    break;
                case "c4":
                    changePercentage(skillTextEl, 65);
                    document.querySelector(".c4").style.strokeDashoffset = 376.99 - (376.99 / 100) * 65;
                    break;
                case "c5":
                    changePercentage(skillTextEl, 85);
                    document.querySelector(".c5").style.strokeDashoffset = 376.99 - (376.99 / 100) * 85;
                    break;
                case "c6":
                    changePercentage(skillTextEl, 80);
                    document.querySelector(".c6").style.strokeDashoffset = 376.99 - (376.99 / 100) * 80;
                    break;
            }
...
const AnimateHeader = () => {
    ...
    const index = Math.floor(Math.random() * 11) % icons.length;
